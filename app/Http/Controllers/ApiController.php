...
class ApiController extends Controller
{
     ...
     * @return \Illuminate\Http\JsonResponse
     */
    public function widgetCreateTicket($workspace)
    {
        $invalidFields = array();
        ...
        if ((!isset($_POST['text'])) || (strlen($_POST['text']) <= 5) || (strlen($_POST['text']) > 4096)) {
            $invalidFields[] = array('name' => 'text', 'value' => (isset($_POST['text'])) ? $_POST['text'] : null);
        }
        ...
        if (!$ws->enable_widget) {
            return response()->json(array('code' => 403, 'workspace' => $workspace, 'enable_widget' => false));
        }
